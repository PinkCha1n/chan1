#include "main.h"    // 替换原有的 stm32f10x.h，引入 HAL 库
#include "OLED.h"
#include <string.h>
#include <math.h>
#include <stdio.h>
...
{
	/* Deprecated for hardware I2C: no-op when using HAL I2C1 */
}
...
{
    /* Wait for OLED power supply to stabilize */
    HAL_Delay(100);
    
    /* Release SCL and SDA */
	/* Ensure I2C1 is initialized by MX_I2C1_Init() before calling OLED_Init().
	   GPIO pin configuration for PB8/PB9 is handled in HAL_I2C_MspInit. */
}
...
{
	/* No-op: hardware I2C start is handled by HAL when transmitting */
}
...
{
	/* No-op: hardware I2C stop is handled by HAL when transmitting */
}
...
{
	/* Deprecated: individual byte send performed by HAL in OLED_WriteCommand/Data */
}
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	buf[1] = Command;
	HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)(0x3C << 1), buf, 2, 100);
}
...
{
	/* For hardware I2C, send a buffer starting with control byte 0x40 followed by data */
	if (Count == 0 || Data == NULL) return;
	uint8_t txBuf[129];
	if (Count > 128) Count = 128; /* limit to buffer size */
	txBuf[0] = 0x40; /* control byte: Co = 0, D/C# = 1 => data stream */
	for (uint8_t i = 0; i < Count; i++) txBuf[i + 1] = Data[i];
	HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)(0x3C << 1), txBuf, Count + 1, 200);
}
